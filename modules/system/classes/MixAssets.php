...
class MixAssets
{
    ...
    public function init()
    {
        ...
        */
        ...
        if (count($packages)) {
            foreach ($packages as $pluginCode => $packageArray) {
                ...
                foreach ($packageArray as $name => $package) {
					//TODO: Seperator Error
					// old: $this->registerPackage($name, PluginManager::instance()->getPluginPath($pluginCode) . '/' . $package);
            ...
            if (!is_null($theme)) {
                ...
                if (count($mix)) {
                    foreach ($mix as $name => $file) {
						//TODO: Seperator Error
                        // old: $this->registerPackage($name, $theme->getPath() . '/' . $file);
        ...
        foreach ($enabledModules as $module) {
			...
			//TODO: Seperator Error
            // Old: $path = base_path("modules/$module") . '/' . $this->mixJs;
        ...
        foreach ($plugins as $plugin) {
			//TODO: Seperator Error
            // Old: $path = $plugin->getPluginPath() . '/' . $this->mixJs;
        ...
        if (in_array('Cms', $enabledModules)) {
            ...
            foreach ($themes as $theme) {
				//TODO: Seperator Error
				// Old: $path = $theme->getPath() . '/' . $this->mixJs;
    ...
    public function registerPackage($name, $path)
    {
		...
		//TODO: Seperator Error
		// Old: $path = Str::after($pinfo['dirname'], base_path() . '/');
